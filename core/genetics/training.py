...
class Genetics:
	...
	def __init__(self,
		...
		self.generation["population"] = self._evaluate(self.population)
	...
	def getGenStats(self) -> dict:
		...
		maxAvgScore = scores[np.argmax(scores)]
		...
		bestFitness = fitnesses[np.argmax(fitnesses)]
		return {"bestAvgScore": maxAvgScore, "avgScore": avgScore, "bestAvgFitness": bestFitness, "avgFitness": avgFitness, "top10AvgFitness": top10AvgFitness, "top25AvgFitness": top25AvgFitness}
	...
	def _updateMemberId(self, member):
		"""
		Has member change id and sets new base 36 id to be one more. This method should be safer against overflow
			since it updates digits one by one. Slower than built in numpy method but shouldn't cause noticable
			slow down during run time.
		"""
		member.updateId(self.nextId)
		base36 = list(self.nextId)[::-1]  # d_0, d_1, ..., d_n
		...
		self.nextId = "".join(base36[::-1])
		# self.nextId = np.base_repr(int(self.nextId, 36) + 1, 36)  # faster but could cause overflow if base36 number get much larger than ZZZZZ
	...
	def _makeMutants(self) -> list:
		...
		for _ in range(int(0.1 * self.mutations)):
			...
			mutant = self._mutate(mutant, numMutations=randint(2, 4), updateId=False)
