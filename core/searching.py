...
def longestPath(space, origin, target, pickup, impassable=-1, depth=2, path = None) -> list:
	if path is None:
...
def floodFillCount(space, origin, stopValue=-1, considerOrigin=True, depth=25):
    #print(origin)
    if not depth or (considerOrigin and (origin not in space or space[origin] == stopValue)):
        return 0
